...
data WORK.guar_derived(keep=PROC_DTE AC_CODE CIF_NO MKT_SUB_SEG_DESC RSME_FLG &act_out);
    retain PROC_DTE AC_CODE CIF_NO MKT_SUB_SEG_DESC RSME_FLG &act_out;
    length MKT_SUB_SEG_DESC $60 RSME_FLG $1;
    set LBFRS9.T_MTH_FRS9_GUARANTEE_DTL(keep=PROC_DTE AC_CODE CIF_NO
...
%if %upcase(&mode) = CHECK %then %do;
proc freq data=WORK.guar_derived;
