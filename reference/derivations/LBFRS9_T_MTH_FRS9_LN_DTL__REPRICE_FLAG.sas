...
data WORK.ln_derived(keep=PROC_DTE AC_CODE REPRICE_DATE REPRICE_FLAG &act_out);
    retain PROC_DTE AC_CODE REPRICE_DATE REPRICE_FLAG &act_out;
    length REPRICE_FLAG $1;
    format REPRICE_DATE datetime20.;
    set LBFRS9.T_MTH_FRS9_LN_DTL(keep=PROC_DTE AC_CODE ACCT_STATUS_CODE
                                      REPRICE_DATE &act_keep &act_ren);
    where PROC_DTE >= &rpt_dtm and PROC_DTE < &nxt_dtm
          and ACCT_STATUS_CODE = "Active";

    if not missing(REPRICE_DATE) then REPRICE_FLAG = 'Y';
    else REPRICE_FLAG = 'N';
